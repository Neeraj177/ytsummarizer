...
const ThinkingIndicator = () => (
  <div className="flex items-start gap-3 fade-in">
    <div className="w-7 h-7 rounded-lg border flex items-center justify-center shrink-0" style={{ background: "var(--bg-tertiary)", borderColor: "var(--border-secondary)" }}>
    ...
    <div className="chat-bubble-ai rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-1.5">
      <span className="thinking-dot w-1.5 h-1.5 rounded-full inline-block" style={{ background: "var(--text-tertiary)" }} />
      <span className="thinking-dot w-1.5 h-1.5 rounded-full inline-block" style={{ background: "var(--text-tertiary)" }} />
      <span className="thinking-dot w-1.5 h-1.5 rounded-full inline-block" style={{ background: "var(--text-tertiary)" }} />
    </div>
  </div>
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-5 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[var(--text-primary)]">Contextual AI Assistant</span>
        </div>
        <span
          ...
          style={!isReady ? { background: "var(--bg-tertiary)", borderColor: "var(--border-secondary)" } : undefined}
        >
      ...
      <div className="flex-1 overflow-y-auto min-h-0 space-y-4 pr-1 pb-2">
        ...
        {!loadingHistory && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 py-8 text-center">
            <div className="w-10 h-10 rounded-xl border flex items-center justify-center" style={{ background: "var(--bg-tertiary)", borderColor: "var(--border-secondary)" }}>
              <Bot className="w-5 h-5 text-[var(--text-tertiary)]" />
            </div>
            <div>
            ...
            <div
              className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                msg.role === "user"
                  ...
                  : `chat-bubble-ai rounded-tl-sm ${msg.isError ? "text-red-400" : "text-[var(--text-secondary)]"}`
              }`}
            >
              {msg.content}
            </div>
      ...
      <div className="shrink-0 pt-4 border-t" style={{ borderColor: "var(--border-secondary)" }}>
        <div className="relative flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 rounded-xl px-4 py-3 pr-12 border
        ...
        <p className="text-xs text-[var(--text-muted)] mt-1.5 text-right">Enter to send · Shift+Enter for newline</p>
      </div>
    </div>
  );
}