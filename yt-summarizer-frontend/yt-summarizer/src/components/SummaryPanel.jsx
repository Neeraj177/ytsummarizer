    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-5 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[var(--text-primary)]">Technical Summary</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {!summary && (status === "PENDING" || status === "PROCESSING") && (
          <div className="flex flex-col items-center justify-center h-full gap-4 py-12">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-[var(--text-tertiary)]">This may take a minute for longer videos</p>
            </div>
            {/* Skeleton lines */}
            <div className="w-full max-w-xs space-y-2 mt-4">
              {[100, 80, 90, 65, 75].map((w, i) => (
                <div
                  ...
                  style={{ width: `${w}%`, animationDelay: `${i * 0.15}s`, background: "var(--bg-tertiary)" }}
                />
              ))}
            </div>
          </div>
        ...
        {status === "FAILED" && (
          <div className="flex flex-col items-center justify-center h-full gap-3 py-12">
            ...
            <p className="text-sm text-[var(--text-secondary)] text-center">Processing failed for video <span className="text-[var(--text-primary)] font-mono">{videoId}</span>.</p>
          </div>
        )}
        ...
        {summary && (
          <div className="fade-in prose-custom">
            <ReactMarkdown
              components={{
                h1: ({ children }) => (
                  <h1 className="text-lg font-bold text-[var(--text-primary)] mb-3 mt-5 first:mt-0 border-b pb-2" style={{ borderColor: "var(--border-primary)" }}>{children}</h1>
                ),
                h2: ({ children }) => (
                  <h2 className="text-base font-semibold text-[var(--text-primary)] mb-2 mt-4 first:mt-0">{children}</h2>
                ),
                h3: ({ children }) => (
                  <h3 className="text-sm font-semibold text-indigo-400 mb-1.5 mt-3">{children}</h3>
                ),
                p: ({ children }) => (
                ...
                code: ({ children }) => (
                  <code className="text-xs text-emerald-400 px-1.5 py-0.5 rounded font-mono" style={{ background: "var(--bg-tertiary)" }}>{children}</code>
                ),
                blockquote: ({ children }) => (
                  <blockquote className="border-l-2 border-indigo-500/50 pl-3 py-0.5 my-3 text-[var(--text-tertiary)] italic text-sm">{children}</blockquote>
                ),
              }}
            >
              {summary}
